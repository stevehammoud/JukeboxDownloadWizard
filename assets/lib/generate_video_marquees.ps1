...
function Normalize-MarqueeArtist {
    ...
    if ($clean -match '(?i)^\s*AC\s*/?\s*DC\s*    if ($clean -match '(?i)^\s*DESTINYS\s+CHILD\s*$') { return "Destiny's Child" }
...
try {
    ...
    if ($clean -match '(?i)^\s*(A\s*\$?\s*AP|ASAP)\s+ROCKY\s*    if ($clean -match '(?i)^\s*DESTINYS\s+CHILD\s*$') { return "Destiny's Child" }
    if ($clean -match '^\s*(?<thousands>\d{1,2})\s+(?<hundreds>\d{3})(?<rest>\s+\S.*)$') {
        $clean = ('{0},{1}{2}' -f $Matches['thousands'], $Matches['hundreds'], $Matches['rest'])
    }
    return $clean
